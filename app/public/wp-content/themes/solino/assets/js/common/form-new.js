...
$.each(dataCity, function( index, value ) {
	...
	$('.persFormC2 select').selectric('refresh');
});
...
$("#fCity").change(function(){
	...
	$('#fDistrict').prop("disabled", true);
});

function sendMondayNow(){
	...
	var tel = $(".persFormC #fPhone").val().replace(/[^0-9\.]/g, '').substring(2);
	//var tel2 = "+90"+tel+"";
	var mail = $(".persFormC #fEmail").val();
	var il = $(".persFormC #fCity").val();
	var ilce = $(".persFormC #fDistrict").val();
	var kurumsal = "2";
	var mustakil = $(".persFormC #mustakil option:selected").text();
	var fatura = $(".persFormC #fatura").val();
	var mesaj = $(".persFormC #fMsg").val();
    if ($('.persFormC #kvkkCheck').prop('checked')) {
        kvkk = $(".persFormC #kvkkCheck").val();
    }
	...
	*/
	...
	monday.api('mutation { create_item(board_id: 4222715493, item_name:  "'+ ad +'"){ id } }').then(res => {
		...
		*/
		monday.api('mutation { change_simple_column_value(item_id: '+ res.data['create_item'].id +', board_id: 4222715493, column_id: "durum82", value: "'+ mustakil +'") {id}}');
		...
		monday.api('mutation {change_multiple_column_values(item_id:'+ res.data['create_item'].id +', board_id:4222715493, column_values:\"{\\\"telefon\\\" : {\\\"phone\\\" : \\\"0'+tel+'\\\", \\\"countryShortName\\\" : \\\"TR\\\"}}\") {id}}');
		...
		monday.api('mutation { change_simple_column_value(item_id: '+ res.data['create_item'].id +', board_id: 4222715493, column_id: "durum41", value: "'+fatura+'") {id}}');  
		monday.api('mutation { change_simple_column_value(item_id: '+ res.data['create_item'].id +', board_id: 4222715493, column_id: "uzun_metin2", value: "'+mesaj+'") {id}}');
        monday.api('mutation { change_simple_column_value(item_id: '+ res.data['create_item'].id +', board_id: 4222715493, column_id: "durum092", value: "'+kvkk+'") {id}}');
...
$(document).ready(function() {
	...
	function dontSendForm1(){
		var mustValueSel = $('#applyFormNew #mustakil').val();
		var fatValueSel = $('#applyFormNew #fatura').val();
		if (mustValueSel == "2"){
			$("#error1").slideDown();
			$("#applyFormNew .formMoreArea").slideUp();
			$("#contact-form").addClass("close");
			$("#applyFormNew input").prop('disabled', true);
			$("#applyFormNew select").prop('disabled', true).selectric('refresh');
			$("#applyFormSend").prop('disabled', true);
			$("#applyFormNew textarea").prop('disabled', true);
			$("#applyFormNew #mustakil").prop('disabled', false).selectric('refresh');
			//$("#applyFormNew .errItem .error").empty();
		}
		else {
			$(".optErrorBox").fadeOut();
			$("#applyFormNew input").prop('disabled', false);
			$("#applyFormNew select").prop('disabled', false).selectric('refresh');
			$("#applyFormSend").prop('disabled', false);
			$("#applyFormNew textarea").prop('disabled', false);
			$("#contact-form").removeClass("close");
			$("#error1").slideUp();
			$("#applyFormNew .formMoreArea").slideDown();
			$('#applyFormNew #fatura').trigger("reset");
		}
		return false;
	}
	
	function dontSendForm2(){
		var mustValueSel = $('#applyFormNew #mustakil').val();
		var fatValueSel = $('#applyFormNew #fatura').val();
		if (mustValueSel == "1" && fatValueSel == "2.000 TL ve altı"){
			$("#error2").slideDown();
			$("#applyFormNew .formMoreArea").slideUp();
			$("#contact-form").addClass("close");
			$("#applyFormNew input").prop('disabled', true);
			...
			$("#applyFormNew textarea").prop('disabled', true);
			$("#applyFormNew #mustakil").prop('disabled', false).selectric('refresh');
			$("#applyFormNew #fatura").prop('disabled', false).selectric('refresh');
			//$("#applyFormNew .errItem .error").empty();
		}
		else {
			$("#applyFormNew input").prop('disabled', false);
	...
	$('#applyFormNew #mustakil').on('change', function (e) {
		dontSendForm1();
	});
	
	$('#applyFormNew #fatura').on('change', function (e) {
		dontSendForm2();
	});
    ...
    */
	...
	$(".persFormC.newF #applyFormNew").validate({
		rules: {
			fName: {
				required: true,
				minlength: 5,
