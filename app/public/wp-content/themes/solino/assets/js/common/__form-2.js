...
$.each(data, function( index, value ) {
	$('.corpFormC #fCity').append($('<option>', {
		value: value.il,
		text:  value.il
	}));
	$('.corpFormC select').selectric('refresh');
});

$(".corpFormC #fCity").change(function(){
	var valueSelected = this.value;
	if($('.corpFormC #fCity').val() > "") {
	  $('.corpFormC #fDistrict').html('');
	  $('.corpFormC #fDistrict').append($('<option>', {
		value: "",
		text:  'Lütfen bir ilçe seçiniz'
	  }));
	  $('.corpFormC #fDistrict').prop("disabled", false);
	  
	  var resultObject;
	  $.each(data, function(i, v) {
		if (v.il == $('.corpFormC #fCity').val()) {
			resultObject = v;
			//alert(v);
			return;
		}
	...
	$('.corpFormC #fDistrict').prop("disabled", true);
});
...
function sendMonday(){
	...
	var ad = $(".corpFormC #fName").val();
	var tel = $(".corpFormC #fPhone").val().replace(/-/g, "").replace(/ /g, "").replace(/\(|\)/g, "");
	var mail = $(".corpFormC #fEmail").val();
	var il = $(".corpFormC #fCity").val();
	var ilce = $(".corpFormC #fDistrict").val();
	var kurumsal = "1";
	var arazi = $(".corpFormC #arazi").val();
	...
	monday.api('mutation { create_item(board_id: 4222715493, item_name:  "'+ ad +'"){ id } }').then(res => {
		...
		//
		if ((arazi != "0" && kurumsal == "Kurumsal")) {
			monday.api('mutation { change_simple_column_value(item_id: '+ res.data['create_item'].id +', board_id: 4222715493, column_id: "durum_1", value: "'+ arazi +'") {id}}');
		} 
		...
		$('.corpFormC #formSuccess').html("Form başarıyla gönderilmiştir. Teşekkür ederiz. (Kurumsal)").fadeIn();
